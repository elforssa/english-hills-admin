...
function ReviewItem({ item, onDone }) {
  const learnerOptional = item.learner_optional === true;
    ...
    try {
      ...
      await crmRpc('crm_resolve_external_intake', { p_request: crypto.randomUUID(), p_submission: item.id,
        ...
        p_data: candidate ? { lead_id: candidate.id, expected_version: candidate.version } : action === 'new' || contact
          ? { contact_name: name.trim(), learner_name: learner.trim() || (learnerOptional ? null : ''), program_interest_text: program.trim(), ...(contact ? { contact_id: contact.id, contact_version: contact.version } : {}) } : {} });
  ...
  return <details className="border-t border-slate-100 px-4 py-3" data-testid="intake-review-item">
    ...
    <div className="mt-3 space-y-3 text-sm">
      ...
      {(action === 'new' || action.startsWith('contact:')) && <div className="grid gap-3 sm:grid-cols-3">{[['Contact', name, setName], [learnerOptional ? 'Apprenant (facultatif)' : 'Apprenant', learner, setLearner], ['Programme', program, setProgram]].map(([label, value, setter]) => <label key={label}>{label}<input aria-label={`${label} de la demande`} maxLength={200} className="mt-1 h-11 w-full rounded-md border px-3" value={value} onChange={e => setter(e.target.value)} /></label>)}</div>}
      <Button size="sm" disabled={busy || (action === 'new' || action.startsWith('contact:')) && (!name.trim() || !program.trim() || (!learnerOptional && !learner.trim()))} onClick={resolve}>{busy ? 'Traitement…' : 'Confirmer la décision'}</Button>
    </div>
  </details>;
}
