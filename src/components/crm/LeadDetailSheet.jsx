...
export default function LeadDetailSheet({
 ...
 <ReadState query={detail} empty="Ce prospect n’est plus disponible.">{lead && <div className="mt-5 space-y-5">
  ...
  {active && <>
   <section className="rounded-xl border border-blue-100 bg-blue-50/60 p-5"><p className="text-xs font-semibold uppercase tracking-wider text-blue-800">Prochaine action</p><p className="mt-2 font-semibold">{lead.next_task ? TASKS[lead.next_task.task_type] || 'Action prévue' : lead.next_placement ? 'Test de niveau prévu' : lead.unreachable_eligible ? '5 appels infructueux effectués' : 'Choisir la prochaine étape'}</p>{(lead.next_task || lead.next_placement) && <p className="mt-1 text-sm text-slate-600">{dateLabel(lead.next_task?.due_at || lead.next_placement?.scheduled_for)}</p>}{lead.post_test_result && <p className="mt-1 text-sm">Niveau recommandé : {lead.post_test_result.niveau_recommande}</p>}<div className="mt-3 flex flex-wrap gap-2">{lead.status === 'QUALIFIED' && lead.next_task?.task_type === 'confirm_placement_test' && !lead.next_placement ? <Button onClick={() => setPlacement(null)}>Réserver un test</Button> : lead.next_task ? <><Button size="sm" onClick={() => open(CALL_TASKS.includes(lead.next_task.task_type) ? 'call' : 'complete', lead.next_task)}>{CALL_TASKS.includes(lead.next_task.task_type) ? 'Enregistrer un appel' : 'Marquer comme fait'}</Button><Button size="sm" variant="outline" className="min-h-11" onClick={() => open('reschedule', lead.next_task)}>Replanifier</Button><Button size="sm" variant="ghost" onClick={() => open('cancel', lead.next_task)}>Annuler l’action</Button></> : lead.next_placement ? <Button variant="outline" onClick={() => setPlacement(lead.next_placement)}>Voir le test</Button> : lead.unreachable_eligible ? <><Button onClick={() => open('schedule')}>Prévoir un autre appel</Button><Button variant="outline" className="min-h-11" onClick={() => open('unreachable')}>Clôturer : injoignable</Button></> : <Button size="sm" onClick={() => open('schedule')}>Planifier</Button>}</div></section>
