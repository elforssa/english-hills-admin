...
export default function ReceiptForm({ onSubmit, onCancel, saving, initialData = {} }) {
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    <Step number="2" title="Solde existant ou nouvelle session" subtitle="Les conditions enregistrées d’un engagement existant restent inchangées">
      ...
      {form.student_id && charges.length > 0 && <div className="space-y-3"><Field label="Choisir un engagement"><select className={input} value={form.charge_id} onChange={(e) => selectCharge(e.target.value)}><option value="">Créer une nouvelle session</option>{charges.map((charge) => <option key={charge.id} value={charge.id}>{charge.session_type}{charge.school_year ? ` · ${charge.school_year}` : ''} · reste {money(charge.balance)} MAD</option>)}</select></Field>{selectedCharge && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="font-black text-amber-950">{selectedCharge.service_description}</p><p className="mt-1 text-sm text-amber-900">{selectedCharge.session_type}{selectedCharge.school_year ? ` · année ${selectedCharge.school_year}` : ''}{selectedCharge.plan_type ? ` · ${selectedCharge.plan_type}` : ''}</p><div className="mt-3 grid grid-cols-3 gap-2"><MiniAmount label="Prix net" value={selectedCharge.net_amount} /><MiniAmount label="Déjà payé" value={selectedCharge.paid_amount} /><MiniAmount label="Solde" value={selectedCharge.balance} /></div>{selectedCharge.due_date && <p className="mt-2 text-xs text-amber-800">Échéance : {selectedCharge.due_date}</p>}</div>}</div>}
      {!lockedCharge && <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Année scolaire *"><select required className={input} value={form.school_year} onChange={(e) => set('school_year', e.target.value)}>{SCHOOL_YEAR_OPTIONS.map((year) => <option key={year}>{year}</option>)}</select></Field>
