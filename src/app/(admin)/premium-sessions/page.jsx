    ...
    <div className="max-w-7xl mx-auto p-4 lg:p-8">
      ...
      {!loading && !loadError && tab === 'legacy' && (
        <div className="mt-5 rounded-2xl border border-border bg-card overflow-hidden">
          ...
          <div className="divide-y divide-border">{legacySessions.map((session) => <div key={session.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-5 py-3"><div><p className="text-sm font-bold"><PersonLink id={session.student_id}>{studentsById[session.student_id]?.full_name || 'Apprenant'}</PersonLink></p><p className="text-xs text-muted-foreground">{displayDate(session.scheduled_date)} · {String(session.start_time).slice(0, 5)} · {teachersById[session.teacher_id]?.full_name || '—'}</p></div><span className={`self-start rounded-full px-2.5 py-1 text-xs font-bold ${PREMIUM_SESSION_STATUS_COLORS[session.status]}`}>{STATUS_LABELS[session.status]}</span></div>)}{!legacySessions.length && <p className="px-5 py-8 text-center text-xs text-muted-foreground">Aucun historique individuel.</p>}</div>
        </div>
      )}
    </div>
  );
