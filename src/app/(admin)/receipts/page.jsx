...
export default function Receipts() {
  ...
  const downloadSelected = async () => {
    setGenerating(true); const { data } = await getBrowserClient().from('receipts').select('*').in('id', [...selected]); const doc = new jsPDF({ unit: 'mm', format: 'a4' });
    (data || []).forEach((receipt, index) => { if (index) doc.addPage(); buildReceiptPDF(doc, receipt, 20); });
