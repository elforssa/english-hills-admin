...
async function track(id: string | undefined, status: 'sent' | 'failed' | 'skipped', error?: string) {
  ...
  await fetch(`${url}/rest/v1/receipts?id=eq.${encodeURIComponent(id)}${activeFilter}`, {
    method: 'PATCH',
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    // Webhook payloads can be delayed. Always render from the current row and
    // stop if a correction voided/deleted it after the request was queued.
    receipt = await loadCurrentReceipt(receiptId);
    const decision = receiptEmailDecision(receipt);
    if (decision.action === 'skip') {
      if (receipt.email_delivery_status !== 'sent') await track(receiptId, 'skipped', `${decision.reason}.`);
      return Response.json({ skipped: true, reason: decision.reason });
    }
    const apiKey = Deno.env.get('RESEND_API_KEY');
    if (!apiKey) {
      await track(receiptId, 'failed', 'RESEND_API_KEY is not configured');
    ...
    if (error) { await track(String(receipt.id || ''), 'failed', error.message); return Response.json({ error: error.message }, { status: 500 }); }
    await track(String(receipt.id || ''), 'sent'); return Response.json({ success: true, sent_to: receipt.email, id: data?.id });
  } catch (error) { await track(String(receipt?.id || ''), 'failed', (error as Error).message); return Response.json({ error: (error as Error).message }, { status: 500 }); }
});
